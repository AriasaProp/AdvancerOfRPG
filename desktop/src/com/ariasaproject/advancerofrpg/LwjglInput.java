...
final class LwjglInput implements Input {
		...
		SwingUtilities.invokeLater(new Runnable() {
			...
			public void run () {
				...
				JPanel textPanel = new JPanel() {
					private static final long serialVersionUID = 3620040855373633085L;
					@Override
	...
	public int getY () {
		return GraphFunc.app.getGraphics().getHeight() - 1 - Mouse.getY();
	}
	...
	void updateMouse () {
		...
		if (Mouse.isCreated()) {
			...
			while (Mouse.next()) {
				...
				int y = GraphFunc.app.getGraphics().getHeight() - Mouse.getEventY() - 1;
	...
	void updateKeyboard () {
		if (lastKeyCharPressed != 0) {
			keyRepeatTimer -= GraphFunc.app.getGraphics().getDeltaTime();
			if (keyRepeatTimer < 0) {
