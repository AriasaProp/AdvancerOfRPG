...
public class ZipResourceFile {
    ...
    /*
     * The values we return for ZipEntryRO use 0 as an invalid value, so we want to
     * adjust the hash table index by a fixed amount. Using a large value helps
     * insure that people don't mix & match arguments, e.g. to findEntryByIndex().
     */
    ...
    // 2-byte number
    static public int swapEndian(short i) {
    ...
    /**
     * getAssetFileDescriptor allows for ZipResourceFile to directly feed Android
     * API's that want an fd, offset, and length such as the MediaPlayer. It also
     * allows for the class to be used in a content provider that can feed video
     * players. The file must be stored (non-compressed) in the Zip file for this to
     * work.
     *
     * @param assetPath
    ...
    /**
     * getInputStream returns an AssetFileDescriptor.AutoCloseInputStream associated
     * with the asset that is contained in the Zip file, or a standard
     * ZipInputStream if necessary to uncompress the file
     *
     * @param assetPath
    ...
    /*
     * Opens the specified file read-only. We memory-map the entire thing and close
    ...
    void addPatchFile(String zipFileName) throws IOException {
        ...
        /*
         * Perform the traditional EOCD snipe hunt. We're searching for the End of
         * Central Directory magic number, which appears at the start of the EOCD block.
         * It's followed by 18 bytes of EOCD stuff and up to 64KB of archive comment. We
         * need to read the last part of the file into a buffer, dig through it to find
         * the magic number, parse some values out, and use those to determine the
         * extent of the CD. We start by pulling in the last part of the file.
         */
        ...
        /*
         * Scan backward for the EOCD magic. In an archive without a trailing comment,
         * we'll find it on the first try. (We may want to consider doing an initial
         * minimal read; if we don't find it, retry with a second read as above.)
         */
        ...
        /*
         * Grab the CD offset and size, and the number of entries in the archive. After
         * that, we can release our EOCD hunt buffer.
         */
