...
public class DragAndDrop {
	...
	final Array<Target> targets = new Array<Target>();
	final ObjectMap<Source, DragListener> sourceListeners = new ObjectMap<Source, DragListener>();
	Source dragSource;
	Payload payload;
	Actor dragActor;
	...
	public void addSource(final Source source) {
		DragListener listener = new DragListener() {
			...
			public void drag(InputEvent event, float x, float y, int pointer) {
				...
				// Determine the drag actor, remove the old one if it was added by DragAndDrop,
				// and add the new one.
				Actor actor = null;
				if (target != null)
	...
	/**
	 * Returns true if a drag is in progress and the {@link #setDragTime(int) drag
	 * time} has elapsed since the drag started.
	 */
	...
	static abstract public class Target {
		 ...
		 * touch was moved or a drop occurred. This is called even if
		 * {@link #drag(Source, Payload, float, float, int)} returned false.
		 */
		 ...
		 * target's local coordinate system. This is not called if
		 * {@link #drag(Source, Payload, float, float, int)} returned false.
		 */
	 ...
	 * the stage automatically during the drag operation as necessary and they will
	 * only be removed from the stage if they were added automatically. A source
	 * actor can be used a payload drag actor.
	 */
