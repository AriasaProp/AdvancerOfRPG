...
import com.ariasaproject.advancerofrpg.scenes2d.utils.FocusListener.FocusEvent;
...
public class Stage extends InputAdapter implements Disposable {
	final SnapshotArray<TouchFocus> touchFocuses = new SnapshotArray<TouchFocus>(true, 4, TouchFocus.class);
	 ...
	 * the specified pointer and button. Touch focus is added automatically when
	 * true is returned from
	 * {@link InputListener#touchDown(InputEvent, float, float, int, int)
	 * touchDown}. The specified actors will be used as the
	 * {@link Event#getListenerActor() listener actor} and {@link Event#getTarget()
	 * target} for the touchDragged and touchUp events.
	 */
	...
	/**
	 * Removes touch focus for the specified listener, pointer, and button. Note the
	 * listener will not receive a touchUp event when this method is used.
	 */
	 ...
	 * focus. The location of the touchUp is {@link Integer#MIN_VALUE}. Listeners
	 * can use {@link InputEvent#isTouchFocusCancel()} to ignore this event if
	 * needed.
	 */
	...
	public Vector2 stageToScreenCoordinates(Vector2 stageCoords) {
		...
		stageCoords.y = GraphFunc.app.getGraphics().getHeight() - stageCoords.y;
		return stageCoords;
	}
	...
	protected boolean isInsideViewport(int screenX, int screenY) {
		int x1 = GraphFunc.app.getGraphics().getWidth();
		int y1 = GraphFunc.app.getGraphics().getHeight();
		screenY = y1 - 1 - screenY;
		return screenX >= 0 && screenX < x1 && screenY >= 0 && screenY < y1;
	}
