...
public class GestureDetector extends InputAdapter {
	...
	/**
	 * @param halfTapRectangleWidth  half width in pixels of the rectangle around an
	 *                               initial touch event, see
	 *                               {@link GestureListener#tap(float, float, int, int)}.
	 * @param halfTapRectangleHeight half height in pixels of the rectangle around
	 *                               an initial touch event, see
	 *                               {@link GestureListener#tap(float, float, int, int)}.
	 * @param tapCountInterval       time in seconds that must pass for two touch
	 *                               down/up sequences to be detected as consecutive
	 *                               taps.
	 * @param longPressDuration      time in seconds that must pass for the detector
	 *                               to fire a
	 *                               {@link GestureListener#longPress(float, float)}
	 *                               event.
	 * @param maxFlingDelay          time in seconds the finger must have been
	...
	public boolean touchDown(float x, float y, int pointer, int button) {
		...
		if (pointer == 0) {
			pointer1.set(x, y);
			gestureStartTime = GraphFunc.app.getInput().getCurrentEventTime();
			tracker.start(x, y, gestureStartTime);
			if (GraphFunc.app.getInput().isTouched(1)) {
				// Start pinch.
	...
	public boolean touchDragged(float x, float y, int pointer) {
		...
		// update tracker
		tracker.update(x, y, GraphFunc.app.getInput().getCurrentEventTime());
	...
	public boolean touchUp(float x, float y, int pointer, int button) {
		...
		if (pinching) {
			...
			if (pointer == 0) {
				...
				tracker.start(pointer2.x, pointer2.y, GraphFunc.app.getInput().getCurrentEventTime());
			} else {
				// second pointer has lifted off, set up panning to use the first pointer...
				tracker.start(pointer1.x, pointer1.y, GraphFunc.app.getInput().getCurrentEventTime());
			}
			return false;
		}
		...
		// handle fling
		gestureStartTime = 0;
		long time = GraphFunc.app.getInput().getCurrentEventTime();
	...
	static class VelocityTracker {
		...
		/*
		 * private float getSum (float[] values, int numSamples) { numSamples =
		 * Math.min(sampleSize, numSamples); float sum = 0; for (int i = 0; i <
		 * numSamples; i++) { sum += values[i]; } if (numSamples == 0) return 0; return
		 * sum; }
		 */
	}
}
