...
import com.ariasaproject.advancerofrpg.graphics.g3d.utils.TextureProvider.FileTextureProvider;
...
public class Model implements Disposable {
	...
	protected void loadNodes(Iterable<ModelNode> modelNodes) {
		...
		for (ObjectMap.Entry<NodePart, ArrayMap<String, Matrix4>> e : nodePartBones.entries()) {
			...
			for (ObjectMap.Entry<String, Matrix4> b : e.value.entries()) {
				Matrix4 bV = new Matrix4(b.value);
				bV.inv();
				e.key.invBoneBindTransforms.put(getNode(b.key), bV);
			}
		}
	}
	...
	protected Material convertMaterial(ModelMaterial mtl, TextureProvider textureProvider) {
		...
		if (mtl.opacity != 1.f)
			result.set(new BlendingAttribute(TGF.GL_SRC_ALPHA, TGF.GL_ONE_MINUS_SRC_ALPHA, mtl.opacity));
		...
		if (mtl.textures != null) {
			for (ModelTexture tex : mtl.textures) {
				...
				if (textures.containsKey(tex.fileName)) {
					t = textures.get(tex.fileName);
				} else {
					t = textureProvider.load(tex.fileName);
					textures.put(tex.fileName, t);
					disposables.add(t);
				}
