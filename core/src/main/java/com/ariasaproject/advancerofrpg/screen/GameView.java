...
public class GameView extends Scene {
    ...
    public void show() {
        ...
        cam.lookAt(0, 0, 0);
        cam.update();
        appl.modelBatch.environment.add(dl = new DirectionalLight(new Color(1, 1, 1, 1), new Vector3(1f, -.5f, -.5f).nor()));
        ...
        build = modelBuilder.part("Terrain", TGF.GL_TRIANGLES, Usage.Position | Usage.Normal | Usage.TextureCoordinates, new Material(ColorAttribute.createDiffuse(1, 1, 1, 1)));
        ...
        instance[1] = new ModelInstance(ApplicationListener.asset.<Model>get("model/KnightCharacter.g3db"), new Matrix4().scale(0.08f));
        ...
        ibutton.addListener(new ClickListener() {
            ...
            public void clicked(InputEvent e, float x, float y) {
                if (HasL)
                    appl.modelBatch.environment.remove(pl);
                else
                    appl.modelBatch.environment.add(pl);
                HasL = !HasL;
            }
        });
        ...
        ibutton.addListener(new ClickListener() {
            ...
            public void clicked(InputEvent e, float x, float y) {
                if (HasL)
                    appl.modelBatch.environment.remove(dl);
                else
                    appl.modelBatch.environment.add(dl);
                HasL = !HasL;
            }
        });
    ...
    public void render(float delta) {
        ...
        appl.modelBatch.render(cam, instance);
        appl.stage.draw();
    }
