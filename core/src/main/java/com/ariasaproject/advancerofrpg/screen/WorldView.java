...
public class WorldView extends Scene {
    ...
    public void show() {
        ...
        camera.lookAt(0, 0, 0);
        camera.update();
        appl.modelBatch.environment.add(d1 = new DirectionalLight(new Color(1, 1, 1, 1), new Vector3(1f, -.5f, -.5f).nor()));
        // modelBatch.environment.add(d2 = new DirectionalLight(new Color(1, 1, 1, 1),
        // new Vector3(1f, -.5f, .5f).nor()));
        // modelBatch.environment.add(p1 = new PointLight(new Vector3(15, 35, -15), new
        // Color(0, 1, 1, 1), 7f, 250f));
        ModelBuilder modelBuilder = new ModelBuilder();
        modelBuilder.begin();
        modelBuilder.node().id = "Lamp";
        MeshPartBuilder build = modelBuilder.part("Lamp", TGF.GL_TRIANGLES, Usage.Position | Usage.Normal | Usage.ColorPacked | Usage.TextureCoordinates, new Material(ColorAttribute.createDiffuse(Color.YELLOW)));
        ...
        build = modelBuilder.part("Ball", TGF.GL_TRIANGLES, Usage.Position | Usage.Normal | Usage.ColorPacked | Usage.TextureCoordinates, new Material(ColorAttribute.createDiffuse(Color.GREEN)));
        ...
        build = modelBuilder.part("Box", TGF.GL_TRIANGLES, Usage.Position | Usage.Normal | Usage.ColorPacked | Usage.TextureCoordinates, new Material(ColorAttribute.createDiffuse(Color.YELLOW)));
        ...
        build = modelBuilder.part("Terrain", TGF.GL_TRIANGLES, Usage.Position | Usage.Normal | Usage.TextureCoordinates, new Material(ColorAttribute.createDiffuse(0.2f, 0.2f, 0.55f, 1)));
        ...
        build = modelBuilder.part("caps", TGF.GL_TRIANGLES, Usage.Position | Usage.Normal | Usage.ColorPacked | Usage.TextureCoordinates, new Material(ColorAttribute.createDiffuse(Color.RED)));
        ...
        instance[1] = new ModelInstance(ApplicationListener.asset.<Model>get("model/KnightCharacter.g3db"), new Matrix4().scl(0.08f));
        /*
        ...
        camPad.addListener(new InputListener() {
            Vector3 tmpV1 = new Vector3(), tmpV2 = new Vector3();
            //Vectore3 t = new Vector3();
        ...
        ib.addListener(new ClickListener() {
            ...
            public void clicked(InputEvent e, float x, float y) {
                if (hasLight)
                    appl.modelBatch.environment.remove(d1);
                else
                    appl.modelBatch.environment.add(d1);
                hasLight = !hasLight;
            }
        });
        ...
        st.addActor(ib);
        /*
         * ib = new ImageButton(skin, "jump"); ib.addListener(new ClickListener() {
         * boolean hasLight = true;
         *
         * @Override public void clicked(InputEvent e, float x, float y) { if (hasLight)
         * modelBatch.environment.remove(p1); else modelBatch.environment.add(p1);
         * hasLight = !hasLight; } }); ib.setBounds(stWidth - 160, 0, 80, 80);
         * st.addActor(ib);
         */
        ...
        ib.addListener(new ClickListener() {
            @Override
            public void clicked(InputEvent e, float x, float y) {
                appl.modelBatch.environment.setUseShadowMapping(!appl.modelBatch.environment.isUseShadowMapping());
            }
        });
    ...
    public void render(float delta) {
        ...
        appl.modelBatch.render(camera, instance);
