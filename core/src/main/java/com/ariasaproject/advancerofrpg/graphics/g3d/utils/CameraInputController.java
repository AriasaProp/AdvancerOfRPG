...
public class CameraInputController extends GestureDetector {
    ...
    public void update() {
        if (rotateRightPressed || rotateLeftPressed || forwardPressed || backwardPressed) {
            final float delta = GraphFunc.app.getGraphics().getDeltaTime();
    ...
    protected boolean process(float deltaX, float deltaY, int button) {
        if (button == rotateButton) {
            ...
            camera.rotateAround(target, new Vector3(0, 1, 0), deltaX * -rotateAngle);
    ...
    public boolean touchDragged(int screenX, int screenY, int pointer) {
        ...
        if (result || this.button < 0)
            return result;
        final float deltaX = (screenX - startX) / GraphFunc.app.getGraphics().getWidth();
        final float deltaY = (startY - screenY) / GraphFunc.app.getGraphics().getHeight();
        startX = screenX;
        startY = screenY;
    ...
    protected static class CameraGestureListener extends GestureAdapter {
        ...
        public boolean zoom(float initialDistance, float distance) {
            ...
            float w = GraphFunc.app.getGraphics().getWidth(), h = GraphFunc.app.getGraphics().getHeight();
