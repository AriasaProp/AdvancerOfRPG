...
public class ParticleEmitter {
    ...
    public void draw(Batch batch) {
        if (premultipliedAlpha) {
            batch.setBlendFunction(TGF.GL_ONE, TGF.GL_ONE_MINUS_SRC_ALPHA);
        } else if (additive) {
            batch.setBlendFunction(TGF.GL_SRC_ALPHA, TGF.GL_ONE);
        } else {
            batch.setBlendFunction(TGF.GL_SRC_ALPHA, TGF.GL_ONE_MINUS_SRC_ALPHA);
        }
        Particle[] particles = this.particles;
        boolean[] active = this.active;
        for (int i = 0, n = active.length; i < n; i++) {
    ...
    /**
     * Allocates max particles emitter can hold. Usually called early on to avoid
     * allocation on updates. {@link #setSprites(Array)} must have been set before
     * calling this method
     */
     ...
     * {@link com.ariasaproject.advancerofrpg.graphics.g2d.Batch Batch}'s
     * blend function to the alpha-blending default (GL_SRC_ALPHA,
     ...
     * {@link com.ariasaproject.advancerofrpg.graphics.g2d.Batch Batch}'s blend
     * function to the alpha-blending default (GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA)
     * when done drawing. Is true by default. If set to false, the Batch's blend
     * function is left as it was for drawing this ParticleEmitter, which prevents
     * the Batch from being flushed repeatedly if consecutive ParticleEmitters with
