...
ChainShape_M(void, jniCreateLoop)(JNIEnv *env, jobject obj, jfloatArray vals, jint offset,
                                  jint numVertices) {
...
ChainShape_M(void, jniCreateChain)(JNIEnv *env, jobject obj, jfloatArray vals, jint offset,
                                   jint numVertices) {
    jfloat *data = (jfloat *) env->GetPrimitiveArrayCritical(vals, 0);
    b2Vec2 *verticesOut = new b2Vec2[numVertices];
    for (int i = 0; i < numVertices; i++)
